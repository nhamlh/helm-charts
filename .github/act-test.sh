...
check_prerequisites() {
	...
	if [ ! -f "$SECRETS_FILE" ]; then
		echo "Missing $SECRETS_FILE — copy from act.secrets.example and fill in GITHUB_TOKEN"
		exit 1
	fi
...
test_happy_path() {
	...
	local output
	output=$(run_act "$DOCKER_HOST_VAL")
...
main() {
	...
	if [ -z "$DOCKER_HOST_VAL" ]; then
		echo "Could not find a running Docker socket. Start Docker and retry."
		exit 1
	fi
